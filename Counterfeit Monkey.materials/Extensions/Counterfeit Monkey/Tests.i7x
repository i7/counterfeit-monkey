...
A last after starting the virtual machine rule (this is the no pauses rule):
	if the File of Tests exists or the file of Automated Tests exists:
		...
		if glulx system clock is supported:
			start the timer;
		otherwise:
			say "[line break][first custom style][bracket]This interpreter does not provide system clock functionality. Play time will not be shown at the end.[close bracket][roman type][paragraph break]";
		say paragraph break;
